...
export const VFS: Record<string, { type: string; children: string[] }> = {
  ...
  "/projects": {
    ...
    children: [
      ...
      "league_of_legends_drafting_tool.py",
    ],
  },
  "/logs": {
    type: "dir",
...
logEntries.forEach((entry) => {
  ...
  FILE_CONTENT[absoluteFilePath] = () => (
    <div className="mt-2 max-w-3xl animate-in fade-in slide-in-from-left-2 duration-500">
      <div className="border-l-4 border-hacker-green pl-4 py-1 mb-6 bg-hacker-green/5">
        <h1 className="text-white font-bold text-lg md:text-xl uppercase tracking-wide">
          {entry.title}
        </h1>
        <div className="flex justify-between items-baseline mt-1">
          <p className="text-hacker-green text-xs md:text-sm font-mono opacity-90">
            [ LOG_DATE: {entry.date} ]
          </p>
        ...
        {entry.content.map((line, i) => {
          ...
          if (line.startsWith("#")) {
            return (
              <p
                key={i}
                className="text-hacker-green font-bold text-base border-b border-white/10 pb-1 mt-6 mb-2"
              >
                {line.replace("#", "").trim()}
              </p>
            );
          ...
          if (line.startsWith("*") || line.startsWith("-")) {
            return (
              <p key={i} className="flex gap-3 pl-2">
                <span className="text-hacker-green opacity-80 mt-[2px]">
                  {">>"}
                </span>
      ...
      <div className="mt-8 pt-2 border-t border-white/10 flex justify-between items-center opacity-40">
        <p className="text-[10px] font-mono text-hacker-green uppercase">
          EOF_REACHED //
        </p>
      </div>
    </div>
