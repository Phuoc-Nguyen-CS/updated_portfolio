...
export const COMMANDS: Record<string, CommandFunction> = {
    ...
    ls: (args, cwd, sessionFiles): CommandResponse => {
        ...
        const isFile = !!FILE_CONTENT[absolutePath] ||
            Object.values(sessionFiles).some(f => f.path === dirPath && absolutePath.endsWith(fileName));
    ...
    cat: (args, cwd, sessionFiles): CommandResponse => {
        ...
        if (sessionFileKey) {
            return {
                output: (
                    <div className="whitespace-pre-wrap mt-1 border-l-2 border-blue-500/30 pl-4 py-1">
    ...
    rm: (args, cwd, sessionFiles): CommandResponse => {
        ...
        if (FILE_CONTENT[absolutePath] || EXECUTABLES[absolutePath] || VFS[absolutePath]) {
            return { output: <span className="text-red-500">rm: cannot remove '{target}': Permission denied (Read-only)</span> };
        }
